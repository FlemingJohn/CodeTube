...
const GetPitchFeedbackInputSchema = z.object({
  ...
  audioDataUri: z.string().describe("The user's recorded audio answer as a data URI."),
});
export type GetPitchFeedbackInput = z.infer<typeof GetPitchFeedbackInputSchema>;

const FeedbackItemSchema = z.object({
  isPositive: z.boolean().describe("Whether this piece of feedback is positive or an area for improvement."),
  feedback: z.string().describe("The specific feedback point."),
});

const GetPitchFeedbackOutputSchema = z.object({
  transcribedAnswer: z.string().describe("The transcription of the user's audio response."),
  overallFeedback: z.string().describe("A summary of the user's performance."),
  feedbackBreakdown: z.array(FeedbackItemSchema).describe("A list of specific feedback points, both positive and areas for improvement."),
});
export type GetPitchFeedbackOutput = z.infer<typeof GetPitchFeedbackOutputSchema>;
...
export async function getPitchFeedback(
  input: GetPitchFeedbackInput
    ...
    const audioBuffer = Buffer.from(input.audioDataUri.substring(input.audioDataUri.indexOf(',') + 1),'base64');
    const wavAudio = 'data:audio/wav;base64,' + (await toWav(audioBuffer));

  const transcribed = await ai.generate({
    model: 'googleai/gemini-2.5-flash-image-preview',
    prompt: [{media: {url: wavAudio, contentType: 'audio/wav'}}],
  });
  const transcribedAnswer = transcribed.text;

  const feedbackPrompt = ai.definePrompt({
    name: 'getPitchFeedbackPrompt',
